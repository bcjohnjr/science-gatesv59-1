...
assert all(c in can for c in req)
...
def q(x):
    return [float(v) for v in np.quantile(np.asarray(x, float), [.05, .5, .95])]


def ix(f, y):
    return int(np.argmin(np.abs(np.asarray(f.timebounds, float) - float(y))))
...
def paths(extra=0.0, end=2400):
    ...
    for r in can.itertuples(index=False):
        ...
        off[y] = float(
            r.gross_co2_gtco2 + r.permafrost_co2_gtco2
            + r.stored_carbon_reversal_gtco2
        )
        cdr[y] = float(r.cdr_gtco2)
    # Transparent benchmark extension: hold terminal component fluxes constant.
    for y in range(2184, end + 1):
        on[y] = LG + LP + LR - LC - extra
        off[y] = LG + LP + LR
        cdr[y] = LC + extra
    return on, off, cdr
...
def make(end=2400):
    ...
    # Keep FaIR's official species input modes unchanged. Some species (e.g.
    # sulfur) are emissions-only and cannot legally be converted to forcing.
...
def set_future_nonco2_to_preindustrial_inputs(f, species, props):
    """Neutralize future non-CO2 inputs without changing FaIR species modes.

    Inputs through 2026 are untouched. From 2027 onward:
      * emissions-driven non-CO2 species use each configuration's calibrated
        baseline_emissions (not zero, preserving natural baseline emissions);
      * forcing-driven non-CO2 species are set to zero forcing;
      * concentration-driven non-CO2 species are set to calibrated baseline
        concentration;
      * calculated species remain calculated from the modified drivers.

    This preserves a common state at timebound 2026 and lets long-lived gases
    relax dynamically instead of instantaneously deleting their atmospheric
    burden.
    """
    em_mask = np.floor(np.asarray(f.timepoints, float)).astype(int) > NEUTRAL_AFTER_YEAR
    tb_mask = np.asarray(f.timebounds, float) > NEUTRAL_AFTER_YEAR

    for s in species:
        if s in ('CO2 FFI', 'CO2 AFOLU', 'CO2'):
            continue
        mode = props[s]['input_mode']

        if mode == 'emissions':
            arr = np.asarray(
                f.emissions.loc[dict(scenario=SCENARIO, specie=s)], float
            ).copy()
            b = np.asarray(
                f.species_configs['baseline_emissions'].loc[dict(specie=s)], float
            )
            if np.any(~np.isfinite(b)):
                raise ValueError(f'Non-finite baseline_emissions for {s}')
            arr[em_mask, :] = b[None, :]
            f.emissions.loc[dict(scenario=SCENARIO, specie=s)] = arr

        elif mode == 'forcing':
            arr = np.asarray(
                f.forcing.loc[dict(scenario=SCENARIO, specie=s)], float
            ).copy()
            arr[tb_mask, :] = 0.0
            f.forcing.loc[dict(scenario=SCENARIO, specie=s)] = arr

        elif mode == 'concentration':
            arr = np.asarray(
                f.concentration.loc[dict(scenario=SCENARIO, specie=s)], float
            ).copy()
            b = np.asarray(
                f.species_configs['baseline_concentration'].loc[dict(specie=s)], float
            )
            if np.any(~np.isfinite(b)):
                raise ValueError(f'Non-finite baseline_concentration for {s}')
            arr[tb_mask, :] = b[None, :]
            f.concentration.loc[dict(scenario=SCENARIO, specie=s)] = arr

        elif mode == 'calculated':
            continue
        else:
            raise ValueError(f'Unsupported FaIR input_mode={mode!r} for {s}')


def neutral(net, end=2400):
    f, species, props = make(end)
    fill_standard_inputs(f)
    init(f)
    override_carbon(f, net)
    set_future_nonco2_to_preindustrial_inputs(f, species, props)
    f.run(progress=False)
    return f
...
def stats(f, y):
    ...
    return {
        ...
        'tq': q(t),
        'bq': q(c - base),
        'f280': float(np.mean(c <= 280)),
        'fbase': float(np.mean(c <= base)),
    }
...
def attr(a, b, cdr, label):
    ...
    for i, t in enumerate(np.asarray(a.timebounds, float)):
        cumulative = sum(v for y, v in cdr.items() if y <= int(round(t)) - 1)
        # Configuration-wise paired difference FIRST, then ensemble quantiles.
        delta_member_ppm = bb[i] - aa[i]
        fraction_member = (
            delta_member_ppm * conv / cumulative
            if cumulative > 0 else np.full_like(delta_member_ppm, np.nan)
        )
        dq = q(delta_member_ppm)
        fq = q(fraction_member) if cumulative > 0 else [math.nan] * 3
        rows.append([label, t, cumulative, *dq, *fq])
    return pd.DataFrame(
        rows,
        columns=[
            'experiment', 'timebound_year', 'cumulative_cdr_gtco2',
    ...
    for kind in ['absolute_280', 'member_relative_baseline']:
        ...
        if r is not None:
            ...
            row.update(
                ...
                total_post2183_cdr_rate_gtco2_per_year=LC + r,
                canonical_cdr_2026_2183_gtco2=CAN_CDR,
                historical_program_peak_cdr_gtco2_per_year=PEAK,
                exceeds_historical_program_peak=bool(LC + r > PEAK),
                co2_p05=s['cq'][0], co2_p50=s['cq'][1], co2_p95=s['cq'][2],
                co2_minus_baseline_p05=s['bq'][0],
                co2_minus_baseline_p50=s['bq'][1],
                co2_minus_baseline_p95=s['bq'][2],
                fraction_le_280=s['f280'],
                fraction_le_own_baseline=s['fbase'],
                temp_p05=s['tq'][0], temp_p50=s['tq'][1], temp_p95=s['tq'][2],
            )
